...
| `pbs.khaddict.lab` | VM | [Proxmox Backup Server](https://www.proxmox.com/en/proxmox-backup-server). Stores VM backups on a dedicated 500GB disk. Most VMs get a nightly Proxmox snapshot (via a StackStorm workflow) for fast local rollback, plus a weekly vzdump backup to PBS with an offsite sync to kDrive (`role/pbs/files/pbs-datastore-sync.sh`) for actual disaster recovery, with a few exceptions (PBS itself, stateless K8s nodes). The external VPS also backs up here weekly, over the WireGuard tunnel, into its own namespace so a compromised VPS can't touch the VMs' backups (see [documentation/KHADDICT-VPS.md](documentation/KHADDICT-VPS.md#15-offsite-backup-to-pbs)). |
...
| `bookstack.khaddict.lab` | VM | [BookStack](https://www.bookstackapp.com/) wiki for personal SRE/DevOps revision notes (Apache + PHP-FPM + MariaDB, installed via the project's own community Debian install script, not a Salt-managed package install). Exposed publicly at `sre.khaddict.com` via `revproxy`; public read access is granted through BookStack's own "Public" role, editing stays behind login. Salt (`role/bookstack`) manages only the `.env` config (`APP_URL`, `APP_PROXIES` trusting `revproxy`'s IP, `SESSION_SECURE_COOKIE`) via Vault-sourced secrets, not the base install. A second state on the same minion (`role/bookstack/bookstack_sync.sls`) runs a small Python service that mirrors every book/chapter/page to [`khaddict/sre-notes`](https://github.com/khaddict/sre-notes) as Markdown, polling BookStack's own API every 2 minutes. |
...
| `matomo.khaddict.lab` | LXC | [Matomo](https://matomo.org/) web analytics (Caddy + PHP 8.3-FPM + MariaDB), managed by Salt (`role/matomo`). Tracks `khaddict.com`, `blog.khaddict.com`, `media.khaddict.com`, `projects.khaddict.com`, `api.khaddict.com`, `diagram.khaddict.com`, `sre.khaddict.com`, `dashboard.khaddict.com`, `status.khaddict.com`. The first six get the snippet baked into the static HTML at build time in the [`khaddict-com`](https://github.com/khaddict/khaddict-com) repo; the last three each use a different injection mechanism since they're not built by that repo (see [documentation/MATOMO.md](documentation/MATOMO.md#10-tracking-snippet)). Exposed publicly at `matomo.khaddict.com`. |
...
- `global/`: applied to every host in `data/main.yaml`'s Proxmox inventory: networking, SSH hardening, user management, DNS resolution, CA certificate trust, Promtail, node-exporter, blackbox-exporter, Vault client configuration. The external VPS (see "External exposure" below) is excluded by minion ID in `top.sls`, since it isn't Proxmox-managed and several of these states assume that inventory; it does get `global.common.ca` cherry-picked into `role/vps/init.sls` so it can validate Vault/PBS TLS certs for its own offsite backup.
- `role/`: per-service states applied to specific minions: `api`, `bookstack`, `easypki`, `grafana`, `kcli`, `loki`, `matomo`, `netbox`, `pbs`, `pihole`, `prometheus`, `pve`, `registry`, `revproxy`, `saltmaster`, `stackstorm`, `unifi`, `vault`, `vps`
...
**Uptime Kuma** runs directly on the VPS (Node.js + PM2). nginx terminates TLS for `status.khaddict.com` locally and proxies straight to it on `localhost`, entirely bypassing WireGuard/HAProxy, so the status page stays up even if the entire homelab goes down. Public DNS on the VPS never goes through the tunnel, so the alert path stays independent of the homelab's reachability. The exception is `*.khaddict.lab`, routed through the tunnel for the offsite PBS backup below; it doesn't touch public-domain resolution (see [documentation/KHADDICT-VPS.md](documentation/KHADDICT-VPS.md#9-configure-the-wireguard-tunnel-to-the-homelab)).
